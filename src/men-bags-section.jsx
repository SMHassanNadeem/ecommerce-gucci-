import menHero from './assets/manHero.avif'
...
export default function MenSection() {
  const fetchData = async () => {
    const { data, error } = await supabase.from("url").select("*").eq("mainCategory", "men");
  ...
  return (
    <div className='flex flex-col'>
      <div className="flex justify-center items-end m-0 p-0 w-[100vw] h-[50vh] sm:h-[80vh]">
        <img className="m-0 p-0 w-[100%] h-[100%]" src={menHero} alt="" />
        <div className="flex flex-col items-center gap-3 pb-7 absolute text-white">
          <h1 className="text-4xl">Handbags for Man</h1>
        </div>
      </div>
      ...
      <div>
        ...
        <div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 h-auto p-5">
            ...
            {isLoading ? (
              ...
              <div onClick={() => { navigate(`/men-bags/${p.id}`) }} key={p.id} className="bg-[#e7e7e7] p-2 h-[20rem] w-full">
                <img src={p.url} alt="Image Not Found" className="w-[25rem] h-[80%]" />
