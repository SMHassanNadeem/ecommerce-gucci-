...
export default function NavbarHome() {
    ...
    const fetchData = async () => {
        if (!search) return [];
        const { data, error } = await supabase.from("url").select("*").ilike("title", `%${search}%`);
            ...
            >
                ...
                >
                    <div className="flex flex-col gap-3 items-start">
                        <button className="text-xl cursor-pointer" onClick={() => navigate('/men-bags')}>Man</button>
                        <button className="text-xl cursor-pointer" onClick={() => navigate('/women-bags')}>Women</button>
                    </div>
                </DialogTitle>
            </Dialog>
            <Dialog
                ...
                >
                    <div>
                        <form onSubmit={(e) => { e.preventDefault(); refetch() }}>
                            <input value={search} onChange={(e) => setSearch(e.target.value)} type="search" className="border-b-1 border-gray-300 w-full pl-2 text-sm sm:text-xl" placeholder="Search..." />
                        </form>
